...
class PointCloudToXYPose(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            PointCloud2,
            '/pointcloud/world_view',  # Cambia al topic que tengas
        ...
        self.last_centroid = None  # Para almacenar el último centroide publicado
        self.threshold = 0.01  # 1 cm de tolerancia

        self.get_logger().info("🟢 Nodo activo: Publicará solo si el centroide cambia significativamente")
    ...
    def pointcloud_callback(self, cloud_msg):
        ...
        if len(points) == 0:
            self.get_logger().warn("⚠️ No hay puntos válidos en la nube")
            return
        ...
        # Verifica si el cambio es significativo
        if self.last_centroid:
            ...
            if dx < self.threshold and dy < self.threshold:
                # Cambios menores: no publicar
                return

        # Actualiza el último centroide
